...
//        [self addGestureRecognizer:singleTap];
        [_imageView addGestureRecognizer:singleTap]; //修改放大问题
        
        UITapGestureRecognizer *doubleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleDoubleTap:)];
        doubleTap.numberOfTapsRequired = 2;
//        [self addGestureRecognizer:doubleTap];
        [_imageView addGestureRecognizer:doubleTap]; //修改放大问题
    }
    return self;
}
...
//    CGRect imageFrame = CGRectMake(0, 0, boundsWidth, imageHeight * boundsWidth / imageWidth);
    CGRect imageFrame = CGRectMake(0, 0, boundsWidth, boundsHeight); //修改mr.lu（视图放大显示问题）
...
- (void)hide
{
    ...
    [UIView animateWithDuration:duration + 0.1 animations:^{
        ...
        if(frame.origin.y  < 64+50 || frame.origin.y > [UIScreen mainScreen].applicationFrame.size.height - 70)
        {
...
//        _photo.srcImageView.image = _photo.placeholder;
        ...
        if ([self.photoViewDelegate respondsToSelector:@selector(photoViewDismissFinished)]) {
            [self.photoViewDelegate photoViewDismissFinished];
        }
