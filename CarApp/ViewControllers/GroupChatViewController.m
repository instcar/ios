...
@property (retain, nonatomic) Room *roomInfo;
...
- (void)viewDidLoad
{
    ...
    [groupButton setFrame:CGRectMake(0, 0, 70, 44)];
...
//    [nullSeatValeLable setText:[NSString stringWithFormat:@"%d个",0]];
    ...
    //聊天视图
    _bubbleTableView = [[UIBubbleTableView alloc]initWithFrame:CGRectMake(0, KOFFSETY, 320, SCREEN_HEIGHT-64-44-44)];
    ...
    [self.view insertSubview:_bubbleTableView belowSubview:_messageBgView];
    ...
    //聊天输入框/键盘
    _faceToolBar =[[FaceToolBar alloc]initWithFrame:CGRectMake(0.0f,SCREEN_HEIGHT - toolBarHeight,self.view.frame.size.width,toolBarHeight) superView:self.view];
    ...
    [flowHelpView showWithInView:self.view];
    self.flowHelpView = flowHelpView;
    
    _roomInfoView = [[RoomInfoView alloc]initWithFrame:CGRectMake(0, 75, 320, SCREEN_HEIGHT - 64 - 150)];
    _roomInfoView.groupVC = self;
    [self.view insertSubview:_roomInfoView belowSubview:flowHelpView];
    [self refreshRoomInfo];
...
#pragma mark - Inter
- (void)setRoomViewData
{
    __block typeof(self) bSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [APIClient networkGetroomsWithRoom_id:bSelf.roomID success:^(Respone *respone) {
            if (respone.status == kEnumServerStateSuccess)
            {
                self.roomInfo = [[Room alloc] initWithDic:[respone.data valueForKey:@"room"]];
                NSArray *users = [People arrayWithArrayDic:[respone.data valueForKey:@"user"]];
                if (_roomInfo) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [_roomInfoView setRoomInfo:_roomInfo AndUserInfo:users];
                        [_flowHelpView setRoom:_roomInfo];
                    });
                }
            }
...
{
    [APIClient networkGetroomsWithRoom_id:roomid success:^(Respone *respone) {
        if (respone.status == kEnumServerStateSuccess)
        {
...
{
    /*
     *  把mainview删掉了，需要重新计算 by Liang Zhao
     */
